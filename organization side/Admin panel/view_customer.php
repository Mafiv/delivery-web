...
$sql = "SELECT * FROM customer WHERE full_name LIKE '%$term%' ORDER BY id DESC";
  ...
  function createProductBox($product)
  {
      ...
      $addToCartBtn = '<button id="btn' . $product['id'] . '" onclick="remove_from_cart(' . $product['id'] . ')">Delete customer account</button><br>';
      $box .= $addToCartBtn;
      ...
      <script>
          ...
          function remove_from_cart(productId) {
              ...
              var xhr = new XMLHttpRequest();
                xhr.open('POST', 'delete_customer_account.php', true);
                xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
      // Handle the response from the server
                window.location.href =  "http://localhost/delivery-web/organization%20side/Admin%20panel/view_customer.php"
          ...
          function performSearch() {
            ...
            term=searchTerm;
            window.location.href =  "http://localhost/delivery-web/organization%20side/Admin%20panel/view_customer.php?term=" + encodeURIComponent(term);
          }
          function toggleMenu(){
