...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($fullname) || empty($username) || empty($phone) || empty($password) || empty($confirm_password)) {
        ...
        $stmt = $db->prepare("SELECT COUNT(*) FROM delivery_man WHERE username = ?");
        ...
        if ($username_count > 0) {
            ...
            $stmt = $db->prepare("INSERT INTO delivery_man (full_name, username, phone_number, user_password, image_url) VALUES (?, ?, ?, ?, ?)");
